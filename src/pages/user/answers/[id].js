...
function Page() {
  ...
  return (
    <>
      <div className="mt-8 flex justify-center" onClick={() => handleSwitchClick()}>
        <SwitchToggle checked={checked} />
      </div>
      <div className="w-[80%] m-auto mt-8">
        {checked ? '' : questions.map((question, i) => {
          return (
            <Accordion key={i}>
              <AccordionSummary
                ...
                <div className="grid grid-cols-2">
                  <div className="flex flex-col gap-4">
                    {answers.map((answer, idx) => {
                      return (
                        <p key={i} className="border-b-2 border-black">{`${
                          idx + 1
                        }. ${answer[`question${i}`]}`}</p>
                      );
                    })}
